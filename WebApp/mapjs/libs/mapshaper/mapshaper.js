(function () {
  ...
  */
  ...
  api.affine = function (targetLayers, dataset, opts) {
    ...
    var anchorArg = internal.getAffineAnchor({
      arcs: dataset.arcs, layers: [{
        geometry_type: 'point', shapes: targetPoints
      }, {
  ...
  Dbf.initStringField = function (info, arr, name, encoding) {
    ...
    var buffers = arr.map(function (rec) {
      ...
      size = Math.max(size, buf.length) + 1;
      return buf;
    });
    info.size = size;
    info.write = function (i, bin) {
  ...
  */
  ...
  internal.compileCalcExpression = function (lyr, arcs, exp) {
    ...
    calc1 = internal.compileFeatureExpression(exp, lyr, arcs, {
      context: ctx1,
  ...
  internal.exportFileContentNoMap = function (dataset, opts) {
    var outFmt = opts.format = internal.getOutputFormat(dataset, opts),
      exporter = internal.exporters[outFmt],
      files = [];
  ...
  internal.importDataJSON = function (content, opts) {
    var retn = {};
    if (content) {
      if (utils.isString(content)) {
        try {
